...
__global__ void inject_sources_2D_dyn(float *d_po, float *d_ww,
	...
	if (ss < ns) {
		...
		d_po[ s_z      * nxpad + s_x    ] += wa * d_Sw00[ist];
		d_po[(s_z + 1) * nxpad + s_x    ] += wa * d_Sw01[ist];
		d_po[ s_z      * nxpad + s_x + 1] += wa * d_Sw10[ist];
		d_po[(s_z + 1) * nxpad + s_x + 1] += wa * d_Sw11[ist];
...
__global__ void extract_2D_dyn(float *d_dd_pp, 
			int it, int nr,
			int nxpad, int nzpad,
	...
	if (rr < nr){
		...
		int z_comp   = (d_Rjth[irt]) * nxpad;
		int z_comp_1 = (d_Rjth[irt]+1) * nxpad;

		// set recived pressure vals
		
		d_dd_pp[offset + rr] = d_po[z_comp   + (d_Rjra[irt])]   * d_Rw00[irt] +
                                    d_po[z_comp_1 + (d_Rjra[irt])]   * d_Rw01[irt] +
                                    d_po[z_comp   + (d_Rjra[irt]+1)] * d_Rw10[irt] +
                                    d_po[z_comp_1 + (d_Rjra[irt]+1)] * d_Rw11[irt];
...
__global__ void extract_3D_dyn(float *d_dd_pp, 
				   int it, int nr,
				   int nxpad, int nypad, int nzpad,
	...
	if (rr < nr){
		int y_comp = d_Rjy[irt] * nxpad * nzpad;
		int y_comp_1 = (d_Rjy[irt]+1) * nxpad * nzpad;
		int z_comp = d_Rjz[irt] * nxpad;
		int z_comp_1 = (d_Rjz[irt]+1) * nxpad;
		d_dd_pp[offset + rr] = d_po[y_comp   + z_comp   + (d_Rjx[irt])]   * d_Rw000[irt] +
                                       d_po[y_comp   + z_comp_1 + d_Rjx[irt]]     * d_Rw001[irt] +
                                       d_po[y_comp   + z_comp   + (d_Rjx[irt]+1)] * d_Rw010[irt] +
                                       d_po[y_comp   + z_comp_1 + (d_Rjx[irt]+1)] * d_Rw011[irt] +
                                       d_po[y_comp_1 + z_comp   + (d_Rjx[irt])]   * d_Rw100[irt] +
                                       d_po[y_comp_1 + z_comp_1 + d_Rjx[irt]]     * d_Rw101[irt] +
                                       d_po[y_comp_1 + z_comp   + (d_Rjx[irt]+1)] * d_Rw110[irt] +
                                       d_po[y_comp_1 + z_comp_1 + (d_Rjx[irt]+1)] * d_Rw111[irt];

...
main(int argc, char** argv)
{
  ...
  bool verb, fsrf, snap, bnds, expl, dabc, cden, adj;
  ...
  for (it=0; it<nt; it++) {
    ...
    step_forward_3d(u0,u1,vel,rho,fdcoef_d2,fdcoef_d1,nop,nzpad,nxpad,nypad);
    ...
    if (snap && it%jsnap==0) {
      ...
      for (iy=0; iy<sf_n(acy); iy++) {
        ...
        if (bnds) {
	  sf_floatwrite(ptr_slice[0], fdm->nxpad*fdm->nzpad, file_wfl);
	} else {

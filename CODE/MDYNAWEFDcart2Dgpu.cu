...
int main(int argc, char*argv[]) {
    ...
    int nt, nx, nz, ns, nst, nr, nrt, ncs, nb;
    int it;
    float dt, dx, dz;
    float ot, ox, oz;
    ...
    // 3rd axis for receiver rsf is the time axis
    sf_axis ar_3;
    ar_3 = sf_iaxa(Frec, 3);
    nrt = sf_n(ar_3);
    ...
    ss = (pt2d*) sf_alloc(ns * nst, sizeof(*ss));
    rr = (pt2d*) sf_alloc(nr * nrt, sizeof(*rr));

    float *d_Sw00, *d_Sw01, *d_Sw10, *d_Sw11;
    cudaMalloc((void**)&d_Sw00, ns * nst * sizeof(float));
    cudaMalloc((void**)&d_Sw01, ns * nst * sizeof(float));
    cudaMalloc((void**)&d_Sw10, ns * nst * sizeof(float));
    cudaMalloc((void**)&d_Sw11, ns * nst * sizeof(float));
    sf_check_gpu_error("cudaMalloc source interpolation coefficients to device");

    // x and z coordinates of each source
    int *d_Sjx, *d_Sjz;
    cudaMalloc((void**)&d_Sjx, ns * nst * sizeof(int));
    cudaMalloc((void**)&d_Sjz, ns * nst * sizeof(int));
    sf_check_gpu_error("cudaMalloc source coords to device");

    float *d_Rw00, *d_Rw01, *d_Rw10, *d_Rw11;
    cudaMalloc((void**)&d_Rw00, nr * nrt * sizeof(float));
    cudaMalloc((void**)&d_Rw01, nr * nrt * sizeof(float));
    cudaMalloc((void**)&d_Rw10, nr * nrt * sizeof(float));
    cudaMalloc((void**)&d_Rw11, nr * nrt * sizeof(float));
    sf_check_gpu_error("cudaMalloc receiver interpolation coefficients to device");

    // x and z locations of each receiver
    int *d_Rjx, *d_Rjz;
    cudaMalloc((void**)&d_Rjx, nr * nrt * sizeof(int));
    cudaMalloc((void**)&d_Rjz, nr * nrt * sizeof(int));
	...
	pt2dread1(Fsou, ss, ns * nst, 2);
	pt2dread1(Frec, rr, nr * nrt, 2);
	...
	cs = lint2d_make(ns * nst, ss, fdm);

	sf_warning("Source interp coeffs:");
    sf_warning("00:%f | 01:%f | 10:%f | 11:%f", cs->w00[0], cs->w01[0], cs->w10[0], cs->w11[0]);

    cudaMemcpy(d_Sw00, cs->w00, ns * nst * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Sw01, cs->w01, ns * nst * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Sw10, cs->w10, ns * nst * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Sw11, cs->w11, ns * nst * sizeof(float), cudaMemcpyHostToDevice);
    sf_check_gpu_error("copy source interpolation coefficients to device");

    cudaMemcpy(d_Sjz, cs->jz, ns * nst * sizeof(int), cudaMemcpyHostToDevice);
	cudaMemcpy(d_Sjx, cs->jx, ns * nst * sizeof(int), cudaMemcpyHostToDevice);
	sf_check_gpu_error("copy source coords to device");


	// SET RECEIVERS ON THE GPU
	sf_warning("Receiver Count: %d", nr);
	cr = lint2d_make(nr * nrt, rr, fdm);

	cudaMemcpy(d_Rw00, cr->w00, nr * nrt * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Rw01, cr->w01, nr * nrt * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Rw10, cr->w10, nr * nrt * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Rw11, cr->w11, nr * nrt * sizeof(float), cudaMemcpyHostToDevice);
    sf_check_gpu_error("copy receiver interpolation coefficients to device");

    cudaMemcpy(d_Rjz, cr->jz, nr * nrt * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Rjx, cr->jx, nr * nrt * sizeof(int), cudaMemcpyHostToDevice);
	...
	for (it=0; it<nt; it++) {
        ...
        inject_sources_2D_dyn<<<dimGridS,dimBlockS>>>(d_po, d_ww, 
...
#pragma omp parallel for schedule(dynamic) private(x, z) shared(po, h_po, nxpad)
	    ...
	    extract_2D_dyn<<<dimGrid3, dimBlock3>>>(d_dd_pp, it, nr,
			    		    nxpad, nzpad, 
					        d_po, d_Rjx, d_Rjz,

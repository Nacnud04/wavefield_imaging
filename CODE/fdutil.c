...
/*------------------------------------------------------------*/
lint2d lint2d_update(lint2d ca,
                     pt2d*  aa,
                     fdm2d fdm)
/*< update 2D linear interpolation >*/
{
  int   ia;
  float f1,f2;
    ...
    for    (ix=-nbell;ix<=nbell;ix++) {
        for(iz=-nbell;iz<=nbell;iz++) {
            ...
            for (ia=0;ia<ca->n;ia++) {
                ...
                ww[ia] += uu[ ix+ca->jx[ia]   ][ iz+ca->jz[ia]   ] * bell[nbell+ix][nbell+iz] * ca->w00[ia];
                ww[ia] += uu[ ix+ca->jx[ia]   ][ iz+ca->jz[ia]+1 ] * bell[nbell+ix][nbell+iz] * ca->w01[ia];
                ww[ia] += uu[ ix+ca->jx[ia]+1 ][ iz+ca->jz[ia]   ] * bell[nbell+ix][nbell+iz] * ca->w10[ia];
                ww[ia] += uu[ ix+ca->jx[ia]+1 ][ iz+ca->jz[ia]+1 ] * bell[nbell+ix][nbell+iz] * ca->w11[ia];
            }
        }
    }
